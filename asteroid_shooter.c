...
void Draw(const GameState *gs)
{
    ...
    int  row, col, i;
    int  bx, by, ax, px;
    int  elapsed, mins, secs;   /* [ADDED] For live timer display. */

    GotoXY(0, 0);

    /* [ADDED] Calculate elapsed time to show live on HUD. */
    elapsed = (int)difftime(time(NULL), gs->startTime);
    mins    = elapsed / 60;
    secs    = elapsed % 60;

    printf("  Score: %-6d  Speed: %d  Time: %02d:%02d  ESC = Quit     \n",
           gs->score,
...
int ShowGameOverScreen(int finalScore, time_t startTime)   /* [ADDED] startTime parameter. */
{
    /* Declare variable at top. */
    char    key;
    int     elapsed, mins, secs;   /* [ADDED] Timer variables. */

    /* [ADDED] Calculate how long the session lasted. */
    elapsed = (int)difftime(time(NULL), startTime);
    mins    = elapsed / 60;
    secs    = elapsed % 60;
...
int main(void)
{
    ...
    while (restart)
    {
        ...
        restart = ShowGameOverScreen(gs.score, gs.startTime);   /* [ADDED] Pass startTime. */
    }
